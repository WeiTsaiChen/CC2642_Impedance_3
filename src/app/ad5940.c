...
// SPI RW data monitoring
SPIRdata_Type                  SPIRData;
...
void AD5940_Initialize(void)
{
  ...
  // SPI attribute initialization. Using 32-bit for R/W
  CC2642_AD5941_SPI_Attribute(32);
  ...
  SPIRData.ReadREGData = i;
  
  if(i == 0x5501)
  {
    bIsS2silicon = bTRUE;
    SPIRData.CHIPIDCheck = bIsS2silicon;
  }
  ...
  printf("Note: Current Silicon is %s\n", bIsS2silicon?"S2":"S1");
  printf("AD5940LIB Version:v%d.%d.%d\n", AD5940LIB_VER_MAJOR, AD5940LIB_VER_MINOR, AD5940LIB_VER_PATCH);
#endif
}
...
static void __AD5940_ReferenceON(void)
{
  ...
  ref_cfg.Hp1V8BuffEn = bTRUE; // Original: bTRUE, use internal voltage source. If not, use bFALSE in order to use external voltage source.
